...
hypothesis_function = tf.divide(1.0,
                                1.0 + tf.exp(-(z + theta0)))  # write the hypothesis function based on TF methods here
...
cost_function = tf.reduce_mean(
    tf.multiply(-Y, tf.log(hypothesis_function)) - tf.multiply((1 - Y), tf.log(1 - hypothesis_function)))
...
classifier = np.zeros(
    [number_of_labels, number_of_features + 1])  # Based on the above explanation, what code should we put here?
class_probabilities = np.zeros(
    [number_of_samples, number_of_labels])  # Based on the above explanation, what code should we put here?
...
with tf.Session() as session:
    ...
    for i in range(number_of_labels):
        ...
        for j in range(5000):
            ...
            _, cost = session.run([optimizer, cost_function], feed_dict={X: feature_matrix,
                                                                         Y: np.matrix(label).T})
